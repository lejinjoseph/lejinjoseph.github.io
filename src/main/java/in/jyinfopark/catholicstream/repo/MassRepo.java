...
public interface MassRepo extends CrudRepository<Mass,Integer> {
    ...
    @Query(value = "select distinct `language` from mass where `language` is not NULL and day is not NULL and time_ist is not null order by language desc",nativeQuery = true)
